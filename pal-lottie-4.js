(function(){
  ...
  ready(function(){
    console.log("starting lottie");
    const wrap = document.getElementById('lottie-wrap');
    const fx   = document.getElementById('fx');
    if(!wrap || !fx || !window.lottie) return;

    const DPR = Math.max(1, Math.min(2, window.devicePixelRatio||1));
    function fit(){ fx.width=wrap.clientWidth*DPR; fx.height=wrap.clientHeight*DPR; fx.style.width="100%"; fx.style.height="100%"; }
    fit(); addEventListener('resize', ()=>{ fit(); if(gl) gl.viewport(0,0,fx.width,fx.height); });

    // Let Lottie create/manage its own <canvas>
    const anim = lottie.loadAnimation({
      ...
      rendererSettings: {
        ...
        // important for predictable sizing:
        enableRetinaScaling: false
      },
      // pin to a commit if needed: .../pal@<SHA>/wave-then-idle.json
...
void mainImage(out vec4 fragColor, in vec2 P){
  // your existing 28x28 grid logic (unchanged)
  float cells = 28.0;
...
void main(){ vec4 c; mainImage(c, gl_FragCoord.xy); gl_FragColor=c; }`;
    ...
    let srcCanvas=null, started=false;

    // Wait for Lottie to build its own canvas
    anim.addEventListener('DOMLoaded', ()=>{
      // Lottie’s internal canvas
      srcCanvas = anim.renderer.canvas;
      // Ensure it’s sized; if not, wait a tick
      const kick = ()=>{ if(!srcCanvas.width||!srcCanvas.height){ requestAnimationFrame(kick); return; } start(); };
      kick();
    });

    function start(){
      if (started) return; started=true;
      // Draw after each Lottie paint
      anim.addEventListener('enterFrame', ()=>{ requestAnimationFrame(draw); });
      // Force an initial paint
      anim.setSubframe(false);
      anim.goToAndStop(anim.firstFrame ?? 0, true);
    ...
    fx.addEventListener('webglcontextrestored', ()=>{ gl.viewport(0,0,fx.width,fx.height); requestAnimationFrame(draw); });
  });
})();
