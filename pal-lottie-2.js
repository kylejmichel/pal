(function(){
  ...
  ready(function(){
    ...
    // Hidden source canvas for Lottie
...
vec3 pal(int i){
  if(i==0) return vec3(0.0);      // OFF key (pure black now)
...
const int   K=2;
const vec3  GAP_COL=vec3(0.0), OFF_COL=vec3(0.0); // ← pure black OFF pixels
...
void main(){ vec4 c; mainImage(c, gl_FragCoord.xy); gl_FragColor=c; }`;
    ...
    // start only after first painted frame (fixes blank first-load)
    let started=false;
    function start(){ if(started) return; started=true; gl.viewport(0,0,fx.width,fx.height); requestAnimationFrame(draw); }
    anim.addEventListener('data_ready', ()=>{
      const onFirst = ()=>{ anim.removeEventListener('enterFrame', onFirst); start(); };
      anim.addEventListener('enterFrame', onFirst);
    });
    document.addEventListener('visibilitychange', ()=>{ if(!started && !document.hidden) start(); });
