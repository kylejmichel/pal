(function(){
  ...
  ready(function(){
    ...
    const anim = lottie.loadAnimation({
      ...
      path:'https://YOUR-HOST/animation.json' // <- replace with your JSON URL
    });
...
void mainImage(out vec4 fragColor, in vec2 P){
  // compute block size from min dimension
  float block = floor(min(u_resolution.x, u_resolution.y) / u_cells);
  vec2 gridSize = vec2(block * u_cells);
  vec2 offset = 0.5 * (u_resolution - gridSize); // center grid
  ...
  for (int dy=-K; dy<=K; ++dy) {
    ...
    if (any(lessThan(nid, vec2(0.0))) || any(greaterThanEqual(nid, vec2(u_cells)))) continue;
...
void main(){ vec4 c; mainImage(c, gl_FragCoord.xy); gl_FragColor = c; }`;
    ...
    const uCell = gl.getUniformLocation(prog,'u_cells');
    gl.uniform1f(uCell, 28.0); // 28x28 grid
